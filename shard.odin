...
keychain_handle_list :: proc(fd: posix.FD) {
	...
	for block in ([2][][]u8{s.processed, s.unprocessed}) {
		for blob in block {
			...
			fmt.sbprintf(&b, `"%s"`, mcp_json_escape(desc))
			first = false
		}
	}
	strings.write_string(&b, "]}")
...
keychain_auto_spawn :: proc() -> bool {
	...
	if !os.exists(keychain_path) {
		...
		os2.chmod(
			keychain_path,
			{.Read_User, .Write_User, .Execute_User, .Read_Group, .Execute_Group, .Read_Other, .Execute_Other},
		)
