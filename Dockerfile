...
RUN apt-get update && apt-get install -y --no-install-recommends \
    curl ca-certificates unzip clang nodejs npm \
...
WORKDIR /app
RUN cat > /app/test_integration.sh <<'EOF'
#!/bin/bash
set -e
...
EOF
RUN chmod +x /app/test_integration.sh /app/pm2-mcp.sh
