...
test_traverse_layer0_returns_shard_names :: proc(t: ^testing.T) {
	...
	req := Request{op = "traverse", query = "database schema", max_branches = 5, layer = 0}
	...
	testing.expect(t, !strings.contains(result, "Tables and relations"), "L0 must not include thought content")
}
...
test_traverse_layer1_returns_thought_content :: proc(t: ^testing.T) {
	...
	configs := []Test_Shard_Config{
		{
			...
			thoughts = {
				{description = "Database schema overview", content = "Tables and relations for the main database"},
				{description = "SQL query optimization", content = "Index strategies for fast queries"},
			},
		},
		{
			name     = "beta",
	...
	req := Request{op = "traverse", query = "database schema", max_branches = 5, layer = 1}
	result, handled := daemon_dispatch(&node, req)
	testing.expect(t, handled, "traverse must be handled by daemon")
	...
	testing.expect(t, strings.contains(result, "Database schema overview"), "L1 must include thought description from matched shard")
	testing.expect(t, strings.contains(result, "Tables and relations"), "L1 must include thought content from matched shard")
	// Result IDs should be in shard_name/thought_hex format
	testing.expect(t, strings.contains(result, "alpha/"), "L1 result IDs must be prefixed with shard name")
	// Should NOT contain thoughts from non-matching shard
	testing.expect(t, !strings.contains(result, "HTTP request handling"), "L1 must not include thoughts from non-matching shard")
}
...
test_traverse_layer2_follows_related :: proc(t: ^testing.T) {
	...
	configs := []Test_Shard_Config{
		...
		},
		{
			...
			thoughts = {
				{description = "Migration strategy for schema changes", content = "How to safely evolve database schemas"},
			},
		},
	}
	...
	req := Request{op = "traverse", query = "database schema", max_branches = 5, layer = 2}
	result, handled := daemon_dispatch(&node, req)
	testing.expect(t, handled, "traverse must be handled by daemon")
	...
	testing.expect(t, strings.contains(result, "Database schema overview"), "L2 must include thoughts from matched shard")
	// Should contain gamma's thoughts (related to alpha)
	testing.expect(t, strings.contains(result, "Migration strategy"), "L2 must include thoughts from related shard")
	// Should NOT contain beta's thoughts (not matched, not related)
	testing.expect(t, !strings.contains(result, "HTTP request handling"), "L2 must not include thoughts from unrelated shard")
}
...
test_traverse_layer1_budget_truncation :: proc(t: ^testing.T) {
	...
	req := Request{op = "traverse", query = "database schema", max_branches = 5, layer = 1, budget = 20}
	result, handled := daemon_dispatch(&node, req)
	testing.expect(t, handled, "traverse must be handled by daemon")
	testing.expect(t, strings.contains(result, "status: ok"), "traverse L1 with budget must return ok")
	testing.expect(t, strings.contains(result, "truncated: true"), "L1 with budget must set truncated flag")
...
test_traverse_layer0_unchanged :: proc(t: ^testing.T) {
	...
	configs := []Test_Shard_Config{
		{
			...
			thoughts = {
				{description = "Schema doc", content = "Content"},
			},
		},
	}
	...
	req := Request{op = "traverse", query = "database", max_branches = 5}
	result, handled := daemon_dispatch(&node, req)
	testing.expect(t, handled, "traverse must be handled by daemon")
	...
	testing.expect(t, !strings.contains(result, "Schema doc"), "default traverse must not include thought descriptions")
}
