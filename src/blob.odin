...
test_blob_flush_load_roundtrip :: proc(t: ^testing.T) {
	...
	pt1 := Thought_Plaintext {
		description = "roundtrip processed",
		content     = "processed body",
	}
	pt2 := Thought_Plaintext {
		description = "roundtrip unprocessed",
		content     = "unprocessed body",
	}
	...
	if len(loaded.processed) > 0 {
		...
		if rerr == .None {
			testing.expect(
				t,
				rpt.description == "roundtrip processed",
				"processed description must match",
			)
	...
	if len(loaded.unprocessed) > 0 {
		...
		if rerr == .None {
			testing.expect(
				t,
				rpt.description == "roundtrip unprocessed",
				"unprocessed description must match",
			)
		}
	}
